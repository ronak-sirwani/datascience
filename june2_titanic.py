...
# print(dataset['Age'].loc[(dataset['Survived']==1)].count())
# print(dataset['Age'].loc[(dataset['Survived']==0)].count())
target= pd.DataFrame(dataset['Survived'])
dataset= dataset.drop(['Survived','SibSp','Parch','newCabin'],axis=1)
...
pd.to_pickle(model,'titanic.pickle')

feature= ['Pclass','Gender','Age','Fare','Embarked','relatives','individual']
classname= ['0','1']
dot_data = tree.export_graphviz(model, out_file=None, filled=True)

# Draw graph
fig = plt.figure(figsize=(25,20))
dt= tree.plot_tree(model,feature_names=feature,class_names=classname, filled=True)
#print(dt)
print("Accuracy for depth {}:".format(3),metrics.accuracy_score(y_test,y_predict)*100)